...
% Basic checks that the rotlet kernel does what we want
    tol = 1e-13;
    kernel = kernels.rotlet_ewald(tolerance=tol);
...
function test_residual_decay(testCase)
    ...
    kernel = kernels.rotlet_ewald(tolerance=tol);
    level = 0;
    trunc_err = norm(kernel.diffkernel([1 0 0], [0 0 0], [1 1 1], level), inf);
    testCase.verifyLessThan(trunc_err, 1e-12);
end
