classdef stokeslet_pswf3 < kernels.StokesletFourierSplit
% Stokes kernel split using the Hasimoto-like PSWF decomposition
    ...
    methods
        function obj = stokeslet_pswf3(args)
        % Constructor, takes either tolerance or sigma_0 as input
            ...
            if args.tolerance==0
                ...
                obj.c_pswf = c_pswf + 8; % Heuristic
                fprintf('[stokeslet_pswf3] auto-selected c_pswf=%g\n', obj.c_pswf);
            end
            % Init PSWF
        ...
        function gamma_hat = fourier_scaling(self, ksq, level)
            ...
            gamma_hat(supp_mask) = psi(psi_arg_mask) - 1/2*psi_arg_mask.^2.*d2psi(psi_arg_mask);
        end
    end
end
